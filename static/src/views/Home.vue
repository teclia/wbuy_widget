    ...
    <div class="flex flex-col h-screen bg-white dark:bg-gray-900 rounded-lg" v-if="showWidget">
        ...
        <div class="flex items-center px-4 h-[60px] shadow-md fixed top-0 left-0 right-0 z-10" :style="{
            ...
            <div class="relative shrink-0">
                <img :src="wbuy_ai_chatAvatar" alt="Avatar" class="w-12 h-12 rounded-full" />
            ...
            <div v-else>
                ...
                <div v-for="(message, index) in messages" :key="message.id" class="space-y-2"
                    ...
                    <div class="flex items-start" :class="{
                        ...
                        <div class="w-8 mr-2 shrink-0">
                            <img v-if="shouldShowAvatarAndName(index)" :src="message.avatar || defaultAvatar"
                                alt="Avatar" class="w-8 h-8 rounded-full border shadow-lg bg-white shadow-slate-100" />
                        </div>
                        ...
                        <div class="w-full">
                            ...
                            <div class="w-full bg-blue-600 shadow-lg p-2 rounded-b-lg rounded-tr-lg break-words max-w-[500px]"
                                :style="{
                                    background: message.sender === 'user'
                                        ? `linear-gradient(18deg, rgba(${hexToRgb(wbuy_ai_balloonUserGradientStart)}, 0.85), rgba(${hexToRgb(wbuy_ai_balloonUserGradientEnd)}, 0.85))`
                                        : `linear-gradient(18deg, rgba(${hexToRgb(wbuy_ai_balloonAgentGradientStart)}, 0.85), rgba(${hexToRgb(wbuy_ai_balloonAgentGradientEnd)}, 0.85))`,
        ...
        <div v-if="showConfirmationModal"
            ...
            <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 max-w-md text-left">
                <h2 class="text-xl font-semibold mb-4">Você realmente quer finalizar essa conversa?</h2>
    ...
    methods: {
        ...
        setupSocket() {
            ...
            this.socket.on("receiveMessage", (message) => {
                ...
                // Atualiza o título e o avatar se a mensagem for do assistente
                if (message.sender === "assistant") {
                    this.wbuy_ai_chatTitle = message.name || this.wbuy_ai_chatTitle;
                    this.wbuy_ai_chatAvatar = message.avatar || this.wbuy_ai_chatAvatar;
                }
            });
            ...
            this.socket.on("typing", () => {
                ...
                this.typingTimeout = setTimeout(() => (this.isTyping = false), 30000);
            });
        },
        ...
        setupUserId() {
            ...
            if (!userId) {
                userId = this.query?.id || crypto.randomUUID();
        ...
        sendMessage() {
            if (this.newMessage.trim()) {
                ...
                const message = {
                    ...
                    sender: "user",
                    timestamp: new Date().toISOString(),
                    avatar: this.query?.avatar || this.defaultAvatar,
                    name: this.query?.name || "Você",
        ...
        formatMessageText(text) {
            ...
            return text
                ...
                .replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>") // Converte **negrito** em <strong>
                .replace(
                    /\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g,
                    `<button class="btn-link mt-2"
                style="--btn-gradient-start: ${this.wbuy_ai_chatHeaderGradientStart};
                       --btn-gradient-end: ${this.wbuy_ai_chatHeaderGradientEnd};
                       --btn-text-color: ${this.wbuy_ai_chatHeaderTextColor};"
                onclick="window.open('$2', '_blank')">
                $1
            </button>`
                ); // Transforma links Markdown em botões estilizados
        },
        ...
        async confirmCloseChat() {
            // Emite o evento para arquivar o chat antes de desconectar
            if (this.socket && this.socket.connected) {
                this.socket.emit("archiveChat", { wbuy_ai_id: this.$route.params.id, user_id: this.userId });
            }
            ...
            // Reutiliza a função `setupSocket` para configurar a nova conexão
            this.setupSocket();
        ...
        // Escuta mensagens do script pai (usado para outros comandos)
        parentMessage(event) {
            ...
            } else if (action === "PAGE_URL_RESPONSE") {
                this.last_url = url; // Salva a URL recebida

                // Garante que o `wbuy_ai_id` esteja disponível para a chave
                if (this.last_url && this.$route && this.$route.params && this.$route.params.id) {
                    const wbuy_aiId = this.$route.params.id;
                    const storageKey = `${wbuy_aiId}_last_url`;

                    // Salva a URL no localStorage
                    localStorage.setItem(storageKey, url);
                    //console.log(`URL salva no localStorage com chave ${storageKey}:`, url);
                }
            }
        },
    ...
    async mounted() {
        ...
        this.$nextTick(() => {
            if (this.messages.length > 0) {
                setTimeout(() => {
                    this.scrollToBottom();
                }, 300); // Pequeno delay de 300ms antes de rolar para baixo
            }
        });
...
/* Cor dinâmica do botão baseada no chat */
.btn-link {
